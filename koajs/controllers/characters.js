...
const Ctrl = {
  ...
  setCookies: async (ctx, next) => {
    ...
    if (result.id) {
      ...
      ctx.cookies.set(`assettype`, `character`, ctx.state.cookies.client)
      ctx.cookies.set(`assetid`, result.payload.id, ctx.state.cookies.client)
      // ctx.cookies.set(`assettype`, `character`, ctx.state.cookies.clientHttps)
  ...
  delete: async (ctx, next) => {
    ...
    if (result.stmt.changes === 1) {
      ...
      wsActions.characterDelete({ id: ctx.request.body.assetid })
    }
    else {
      result = {
        success: false,
  ...
  update: async (ctx, next) => {
    ...
    if (result.id) {
      ...
      wsActions.characterUpdate(ctx.request.body.character)
